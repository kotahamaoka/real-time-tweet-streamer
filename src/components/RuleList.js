...
const RuleList = () => {
  ...
  const createRule = async e => {
    ...
    try {
      ...
      if (response.data.body.errors) dispatch({ type: "add_errors", payload: response.data.body.errors })
      else {
  ...
  const errors = () => {
    ...
    if (errors && errors.length > 0) {
      return errors.map(error => <ErrorMessage key={error.title} error={error} styleType="negative" />)
    }
  }
  ...
  const rules = () => {
    ...
    if (!isLoading) {
      if (rules && rules.length > 0) {
        return rules.map(rule => <Rule key={rule.id} data={rule} onRuleDelete={id => deleteRule(id)} />)
      } else {
        return <ErrorMessage key={message.title} error={message} styleType="warning" />
      }
    } else {
      return <Spinner />
    }
    ...
    <div>
      <form onSubmit={e => createRule(e)}>
        <div className="ui fluid action input">
          <input type="text" autoFocus={true} value={state.newRule} onChange={e => dispatch({ type: "rule_changed", payload: e.target.value })} />
